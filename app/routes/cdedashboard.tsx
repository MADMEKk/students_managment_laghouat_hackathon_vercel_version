...
function CDEDashboard() {
  ...
  // Check if the user is authenticated and has the CDE role
  useEffect(() => {
    ...
    if (!hasRole('CDE')) {
      ...
      const dashboardRoutes: Record<Exclude<User['role'], 'CDE'>, string> = {
      ...
      if (user && user.role !== 'CDE' && dashboardRoutes[user.role]) {
  ...
  const fetchProjects = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const data = await projectService.getCDEProjects(token);
  ...
  // Load projects directed to CDE
  useEffect(() => {
    if (isAuthenticated && hasRole('CDE') && token) {
      fetchProjects();
    }
  ...
  const handleTrainingProgramUpload = async () => {
    ...
    try {
      ...
      await projectService.uploadTrainingProgram(file, trainingDates, token || '');
  ...
  const handleExportProjects = async () => {
    ...
    try {
      ...
      try {
        ...
        a.download = `cde-projects-${new Date().toISOString().split('T')[0]}.${fileExtension}`;
  ...
  if (!isAuthenticated || !hasRole('CDE')) {
    return null;
  }
          ...
          <main className="space-y-8">
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-6 rounded-lg shadow-md">
              ...
              <div className="space-y-2">
                ...
                <p className="dark:text-gray-300"><span className="font-medium">Role:</span> CDE Manager</p>
              </div>
            </div>
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-6 rounded-lg shadow-md">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold dark:text-gray-500">
                  {selectedProject ? 'Project Details' : 'Projects Directed to CDE'}
                </h2>
                {!selectedProject && (
                ...
                <div>
                  {loading ? (
                    ...
                    <div className="py-12 text-center text-gray-500 dark:text-gray-400">
                      No projects directed to CDE at the moment.
                    </div>
                  )}
                </div>
              )}
