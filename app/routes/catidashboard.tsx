...
function CATIDashboard() {
  ...
  // Check if the user is authenticated and has the CATI role
  useEffect(() => {
    ...
    if (!hasRole('CATI')) {
      ...
      const dashboardRoutes: Record<Exclude<User['role'], 'CATI'>, string> = {
      ...
      if (user && user.role !== 'CATI' && dashboardRoutes[user.role]) {
  ...
  const fetchProjects = useCallback(async (forceRefresh = false) => {
    ...
    try {
      setLoading(true);
      // Get projects - filtering is now handled in the service
      const data = await projectService.getCATIProjects(token);
  ...
  // Load projects directed to CATI
  useEffect(() => {
    if (isAuthenticated && hasRole('CATI') && token) {
      fetchProjects();
    }
  ...
  const handleTrainingProgramUpload = async () => {
    ...
    try {
      ...
      await projectService.uploadTrainingProgram(file, trainingDates, token || '', 1);
  ...
  const handleExportProjects = async () => {
    ...
    try {
      ...
      try {
        ...
        a.download = `cati-projects-${new Date().toISOString().split('T')[0]}.${fileExtension}`;
  ...
  if (!isAuthenticated || !hasRole('CATI')) {
    return null;
  }
          ...
          <main className="space-y-8">
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-6 rounded-lg shadow-md">
              ...
              <div className="space-y-2">
                ...
                <p className="dark:text-gray-300"><span className="font-medium">Role:</span> CATI Manager</p>
              </div>
            </div>
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-6 rounded-lg shadow-md">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold dark:text-gray-500">
                  {selectedProject ? 'Project Details' : 'Projects Directed to CATI'}
                </h2>
                {!selectedProject && (
                ...
                <div>
                  {loading ? (
                    ...
                    <div className="py-12 text-center text-gray-500 dark:text-gray-400">
                      No projects directed to CATI at the moment.
                    </div>
                  )}
                </div>
              )}
